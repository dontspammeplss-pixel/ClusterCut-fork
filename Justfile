...
# A3 hygiene lint: clipboard payload content must never appear in logs.
hygiene:
    #!/usr/bin/env bash
    set -euo pipefail
    if rg -n "Decrypted Clipboard from.*text" src-tauri/src; then
        echo "HYGIENE-FAIL: payload-content log present"
        exit 1
    fi
    echo "LINT-OK"
