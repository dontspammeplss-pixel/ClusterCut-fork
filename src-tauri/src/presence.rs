...
///
/// Deliberately NOT trust-filtered: untrusted gossip entries are dialed for
/// convergence (a dial is an mTLS handshake attempt, not trust — the peer
/// stays untrusted, runtime-only, never persisted per
/// `storage::persistable_known_peers`, and never satisfies sender
/// verification per `handlers::resolve_sender_device_id`).
...
pub(crate) fn merge_cluster_membership(
    ...
    for peer in &info.known_peers {
        ...
        p.is_trusted = false; // never auto-trust gossip; explicit trust action required
        p.is_manual = false;
...
mod tests {
    ...
    fn merge_imports_new_fingerprinted_member() {
        ...
        *state.local_device_id.lock().unwrap() = "clustercut-me".to_string();
        ...
        assert!(!entry.is_trusted); // H-3: gossip imports stay untrusted until explicit trust
    ...
    fn gossip_import_is_untrusted() {
        // H-3: gossip imports are never auto-trusted — explicit user trust
        // action required. A fresh fingerprinted entry must import as
        // untrusted + non-manual (so the PeerDiscovery persist gate
        // `if peer.is_manual` never writes it to disk).
        let state = AppState::new();
        *state.local_device_id.lock().unwrap() = "clustercut-me".to_string();
        let info = cluster_info(vec![peer("clustercut-new", "192.168.1.20", 4654)]);
        ...
        assert_eq!(imported.len(), 1);
        assert!(!imported[0].is_trusted);
        assert!(!imported[0].is_manual);
        let kp = state.known_peers.lock().unwrap();
        assert!(!kp["clustercut-new"].is_trusted);
    }

    #[test]
    fn merge_membership_never_auto_trusts() {
        // Wiring lock: merge_cluster_membership must not set
        // `is_trusted = true` on gossip imports. Restoring the transitive
        // trust assignment fails here. (AppState-based behavior is covered
        // by gossip_import_is_untrusted above.)
        let src = include_str!("presence.rs");
        let fn_start = src
            .find("pub(crate) fn merge_cluster_membership(")
            .expect("merge_cluster_membership must exist");
        ...
        assert!(
            !body.contains("is_trusted = true"),
            "merge_cluster_membership must never auto-trust gossip imports"
        );
    }
