...
pub(crate) fn perform_factory_reset(app_handle: &tauri::AppHandle, state: &AppState, port: u16) {
    ...
    // 2. Update Runtime State
    {
        ...
        *nn = new_name_val.clone();
        // Wipe the in-memory PIN before replacing it (A3). The dropped
        // Zeroizing value would zeroize on Drop anyway; explicit is better.
        np.zeroize();
        *np = zeroize::Zeroizing::new(new_pin_val);
