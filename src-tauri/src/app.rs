...
/// Late-bound file writer for the log layer (A3). The tracing subscriber is
/// installed in `init_logging` before any `AppHandle` exists, but the log dir
/// must resolve via `app.path().app_log_dir()`. The layer therefore writes
/// through this handle, which sinks until `init_log_file` installs the real
/// rolling appender from inside `setup` (stdout is unaffected throughout).
#[derive(Clone, Default)]
struct LateFileWriter(
    std::sync::Arc<std::sync::Mutex<Option<tracing_appender::rolling::RollingFileAppender>>>,
);

impl std::io::Write for LateFileWriter {
...
impl<'a> tracing_subscriber::fmt::MakeWriter<'a> for LateFileWriter {
    type Writer = Self;
...
static LOG_FILE_WRITER: std::sync::OnceLock<LateFileWriter> = std::sync::OnceLock::new();

fn log_file_writer() -> LateFileWriter {
    LOG_FILE_WRITER
        .get_or_init(LateFileWriter::default)
        .clone()
}

/// Point file logging at the platform log dir with owner-only permissions.
/// Call once from inside `setup`, where the `AppHandle` is available.
fn init_log_file(app_handle: &tauri::AppHandle) {
    let log_dir = app_handle
        .path()
        .app_log_dir()
        .unwrap_or_else(|_| std::env::temp_dir().join("ClusterCutLogs"));
    crate::storage::set_dir_owner_only(&log_dir);
    let appender = tracing_appender::rolling::daily(&log_dir, "clustercut.log");
    *log_file_writer().0.lock().unwrap() = Some(appender);
    // Best-effort 0600 on pre-existing log files. New files land 0600 via
    // the 0077 umask set in `init_logging`; the 0700 dir bounds exposure.
    if let Ok(entries) = std::fs::read_dir(&log_dir) {
        for entry in entries.flatten() {
            let p = entry.path();
            if p.is_file() {
                crate::storage::set_owner_only(&p);
            }
        }
    }
}
...
fn init_logging() -> Args {
    // Owner-only default for every file/dir this process creates from here
    // on (log rotation included): files 0600, dirs 0700. Explicit chmods
    // remain the primary mechanism; this is the backstop.
    // SAFETY: called early in run() before spawning worker threads; explicit
    // chmods remain primary, this umask is the backstop.
    #[cfg(unix)]
    unsafe {
        libc::umask(0o077);
    };
    ...
    // The subscriber is installed here, before any AppHandle exists, but the
    // log dir must come from `app.path().app_log_dir()` (A3: out of the
    // shared temp dir, owner-only). So the layer writes through a late-bound
    // writer that sinks until `init_log_file` installs the real appender
    // from inside `setup`. Stdout shows everything throughout.
    let file_appender = log_file_writer();
...
fn clear_cache(app: &tauri::AppHandle) {
    if let Ok(root_cache_dir) = app.path().app_cache_dir() {
        ...
        if func_exists(&cache_dir) {
            ...
            crate::storage::set_dir_owner_only(&cache_dir);
        }
    }
...
pub(crate) fn run() {
    ...
    builder
        ...
        .manage(AppState::new())
        .setup(move |app| {
            // A3: point file logging at app_log_dir() with owner-only perms.
            ...
            // Load State
            {
                ...
                *state.network_pin.lock().unwrap() = zeroize::Zeroizing::new(network_pin);
            ...
            transport.start_listening(
                move |data, addr, sender_fingerprint| {
                    ...
                    tauri::async_runtime::spawn(async move {
                         match serde_json::from_slice::<Message>(&data) {
                              Ok(msg) => crate::handlers::handle_message(msg, addr, sender_fingerprint, listener_state, listener_handle, transport_inside).await,
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            crate::commands::system::install_gnome_extension,
