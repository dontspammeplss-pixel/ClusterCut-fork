...
pub struct FileStreamHeader {
    ...
    /// Declared plaintext byte count. Inbound: the receiver rejects headers
    /// declaring more than `MAX_FILE_STREAM_BYTES` (2GiB) before creating
    /// any file, and aborts + removes the partial if the stream itself
    /// passes `min(file_size, MAX_FILE_STREAM_BYTES)`.
    pub file_size: u64,
    ...
    // Inbound: decompressed bytes count against MAX_DECOMP_BUDGET (2GiB);
    // the header line itself is capped at MAX_FILE_STREAM_HEADER (8KB) and
    // each read is wrapped in STREAM_READ_TIMEOUT (30s).
    #[serde(default)]
    pub compressed: bool,
