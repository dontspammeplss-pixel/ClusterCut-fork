...
impl Transport {
    ...
    where
        F: Fn(Vec<u8>, SocketAddr, Option<String>) + Send + Sync + 'static + Clone,
                ...
                match connection {
                    Ok(conn) => {
                        ...
                        if proto == "clustercut-file" {
                            ...
                            // Cert-bound sender identity: the handshake already
                            // pinned this cert, so its SHA-256 is the sender id
                            // downstream handlers match on. None if unavailable.
                            let peer_fingerprint: Option<String> = conn
                                .peer_identity()
                                .and_then(|id| id.downcast::<Vec<rustls::pki_types::CertificateDer<'static>>>().ok())
                                .and_then(|certs| certs.first().map(|c| fingerprint_hex(&cert_fingerprint(c.as_ref()))));
                            tauri::async_runtime::spawn(async move {
                                    ...
                                    match conn.accept_bi().await {
                                        Ok((_, mut recv)) => {
                                            ...
                                            match recv.read_to_end(MESSAGE_BYTE_CAP).await {
                                                Ok(buf) => {
                                                    if !buf.is_empty() {
                                                        on_receive_message(buf, remote_addr, peer_fingerprint.clone());
                                                    }
                                                }
...
/// Full lowercase hex of bytes (e.g. a 32-byte cert SHA-256 → 64 chars).
/// Used for sender identity plumbing where truncation would risk collisions.
