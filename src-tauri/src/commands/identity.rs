...
pub(crate) fn get_network_pin(state: State<'_, AppState>) -> String {
    state.network_pin.lock().unwrap().as_str().to_string()
}
...
pub(crate) fn set_network_identity(
    ...
    *state.network_pin.lock().unwrap() = zeroize::Zeroizing::new(pin.clone());
...
pub(crate) fn regenerate_network_identity(
    ...
    *state.network_pin.lock().unwrap() = zeroize::Zeroizing::new(pin);
